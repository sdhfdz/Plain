...
public class HttpEngine {
    ...
    public final static String SERVER_URL = "http://210.28.188.103:10086";
    ...
    public <T> T getHandle(Map<String, String> paramsMap, Type typeOfT) throws Exception {
        String path = SERVER_URL + paramsMap.get("method");
        ;
//        Log.e(TAG,"size = " + paramsMap.size());
        if (paramsMap.size() > 1) {
            for (String key : paramsMap.keySet()) {
                if (key.equals("method")) {
                    continue;
                }
                URLEncoder.encode(paramsMap.get(key), ENCODE_TYPE);
        ...
        conn.setReadTimeout(TIME_OUT);// 设置响应超时, 连接上了,但服务器迟迟不给响应
        conn.connect();// 连接服务器
        InputStream inStream = conn.getInputStream();

        String result = changeInputStream(inStream, ENCODE_TYPE);
        Log.e(TAG, result);
        Gson gson = new Gson();
        return gson.fromJson(result, typeOfT);
